...
struct
  ...
  let run_callbacks
    ...
    let add_callback_to_fuzz callbacks f = f :: callbacks in

    let run_cancel_callbacks fs =
      let callbacks_to_fuzz : (unit -> unit) list ref = ref []  in
      let rec iter_callback_list fs rest =
        match fs with
        | Cancel_callback_list_empty ->
          iter_list rest
        | Cancel_callback_list_callback (storage, f) ->
          callbacks_to_fuzz := add_callback_to_fuzz !callbacks_to_fuzz (fun () -> current_storage := storage; handle_with_async_exception_hook f ());
          iter_list rest
        | Cancel_callback_list_remove_sequence_node node ->
          callbacks_to_fuzz := add_callback_to_fuzz !callbacks_to_fuzz (fun () -> Lwt_sequence.remove node);
          iter_list rest
      ...
      (* Printf.printf "cancel_callbacks - %d\n%!" (List.length !callbacks_to_fuzz); *)
      Lwt_afl_scheduler.fuzz !callbacks_to_fuzz;
    in
    ...
    let run_regular_callbacks fs =
      ...
      let rec iter_callback_list fs rest =
        ...
        | Regular_callback_list_implicitly_removed_callback f ->
          callbacks_to_fuzz := add_callback_to_fuzz !callbacks_to_fuzz (fun () -> f result);
          iter_list rest
      ...
      iter_callback_list fs [];
      (* Printf.printf "regular_callback - %d\n%!" (List.length !callbacks_to_fuzz); *)
      Lwt_afl_scheduler.fuzz !callbacks_to_fuzz
    in
...
struct
  ...
  let choose ps =
    ...
    | n ->
      nth_resolved ps (Crowbar.sample_from_generator (Crowbar.range n))
  ...
  let pick ps =
    ...
    | n ->
      nth_resolved_and_cancel_pending ps (Crowbar.sample_from_generator (Crowbar.range n))
...
struct
  ...
  let wakeup_paused () =
    if Lwt_sequence.is_empty paused then
      ...
      let fuzz_list  = ref [] in
      let tmp = Lwt_sequence.create () in
      Lwt_sequence.transfer_r paused tmp;
      paused_count := 0;
      Lwt_sequence.iter_l (fun wakener -> 
          fuzz_list := (fun () -> wakeup wakener ()) :: !fuzz_list)
        tmp;
      (* Printf.printf "wakeup paused - %d\n%!" (List.length !fuzz_list); *)
