...
class virtual abstract = object(self)
  method virtual private iter : bool -> unit
  ...
  val fds_to_fuzz : (unit -> unit) list ref = ref [] 
  ...
  method on_readable fd f = 
    ...
    let g () = fds_to_fuzz :=  (fun () -> f ev) :: !fds_to_fuzz in
  ...
  method abstract_iter block =
    self#iter block;
    match !fds_to_fuzz  with
    | [] -> ()
    | fd_list -> 
      Lwt_job_notification.reset_notificatons ();
      (* Printf.printf "ready fds - %d\n%!" (List.length fd_list); *)
      Lwt_afl_scheduler.fuzz fd_list;
      fds_to_fuzz := []
end
...
let iter block = !current#abstract_iter block 
