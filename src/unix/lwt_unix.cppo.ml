...
let () =
  try
    ...
    | "switch" ->
      (* Async_switch is handled as Async_detach everywhere *)
      default_async_method_var := Async_detach
    | str ->
      Printf.eprintf
...
let set_default_async_method am = 
  if(am = Async_switch) then default_async_method_var := Async_detach
  else default_async_method_var := am
...
external start_job : 'a job -> async_method -> int -> bool  = "lwt_unix_start_job"
(* Starts the given job with given parameters. It returns [true]
   if the job is already terminated. *)

[@@@ocaml.warning "-3"] [@@@ocaml.warning "-32"]
...
(* Last boolean is submitted_job *)
let dummy_job = Lwt_sequence.add_r ((Lwt.return_unit),(fun exn-> ignore(exn)), (fun () -> ()), (ref false)) (Lwt_sequence.create ())

let execute_jobs () =
  let job_length = ref 0 in
  Lwt_sequence.iter_l (fun (_,_, _,submitted_job) -> if not !submitted_job then job_length := !job_length + 1) jobs;
  if !job_length = 0 then
    ()
  else
    let fuzz_list : (unit -> unit) list ref = ref [] in
    Lwt_sequence.iter_r (fun (_,_, f, submitted_job) -> 
        if !submitted_job then 
          ()
        else begin
          fuzz_list := f :: !fuzz_list;
          submitted_job := true; 
        end)
      jobs;
    (* Printf.printf "async jobs - %d\n%!" (List.length !fuzz_list); *)
    Lwt_afl_scheduler.fuzz (!fuzz_list)

let () = ignore (Lwt_main.Enter_iter_hooks.add_last (fun () -> execute_jobs ()))

let rec abort_jobs exn =
  match Lwt_sequence.take_opt_l jobs with
  | Some (_, f, _, _) -> f exn; abort_jobs exn
  | None -> ()

let cancel_jobs () = abort_jobs Lwt.Canceled

let wait_for_jobs () =
  Lwt.join (Lwt_sequence.fold_l (fun (w, _ , _, _) l -> w :: l) jobs [])
...
let run_job ?async_method job =
  let async_method = choose_async_method async_method in
  if async_method = Async_none then begin
    try
...
let fork () =
  ...
  | 0 ->
    ...
    let l = Lwt_sequence.fold_l (fun (_, f, _, _) l -> f :: l) jobs [] in
    (* Remove them all. *)
