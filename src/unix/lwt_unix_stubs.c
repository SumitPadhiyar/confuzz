...
typedef struct 
{
  ...
  int done; // whether struct notification_info can be freed or not, acts as bool
} notification_info;

static notification_info** notification_infos = NULL;

/* The size of the notification buffer. */
static long job_notification_count = 0;

/* The index to the last allocated notification cell in the notification buffer. -1 indicates no allocation */
static long job_notification_index = -1;

/* The index to the last notification cell in the notification buffer. */
static long last_job_notification_index = -1;

/* The current job notification mode. */
static enum notification_mode job_notification_mode =
    NOTIFICATION_MODE_NOT_INITIALIZED;
...
static void initialize_notification_infos(long start, long end){
  for(long i = start; i <= end; i++){
    notification_infos[i] = NULL;
  }
}

static void job_init_notifications() {
  job_notification_count = 4096;
  notification_infos =
      (notification_info **)lwt_unix_malloc(job_notification_count * sizeof(notification_info*));

  initialize_notification_infos(0, job_notification_count);
}

static void job_resize_notifications() {
  long new_notification_count = job_notification_count * 2;
  notification_info** new_notifications =
      (notification_info **)lwt_unix_malloc(new_notification_count * sizeof(notification_info*));
  memcpy((void *)new_notifications, (void *)notification_infos,
         job_notification_count * sizeof(notification_info*));
  free(notification_infos);
  notification_infos = new_notifications;
  initialize_notification_infos(job_notification_count, new_notification_count);
  job_notification_count = new_notification_count;
}
...
int job_eventfd_notification_send(int notification_fd) {
  uint64_t buf = 1;
  return write(notification_fd, (char *)&buf, 8);
}

int job_eventfd_notification_recv(int notification_fd) {
  uint64_t buf;
  return read(notification_fd, (char *)&buf, 8);
}
...
value lwt_unix_init_notification() {
  switch (notification_mode) {
    ...
    case NOTIFICATION_MODE_EVENTFD:
      notification_mode = NOTIFICATION_MODE_NONE;
      if (close(notification_fd) == -1) uerror("close_lwt_unix_init_notification", Nothing);
      break;
#endif
    case NOTIFICATION_MODE_PIPE:
      notification_mode = NOTIFICATION_MODE_NONE;
      if (close(notification_fds[0]) == -1) uerror("close_lwt_unix_init_notification", Nothing);
      if (close(notification_fds[1]) == -1) uerror("close_lwt_unix_init_notification", Nothing);
      break;
    case NOTIFICATION_MODE_NOT_INITIALIZED:
      notification_mode = NOTIFICATION_MODE_NONE;
      init_notifications();
...
int job_pipe_notification_send(int notification_fd) {
  char buf;
  return write(notification_fd, &buf, 1);
}

int job_pipe_notification_recv(int notification_fd) {
  char buf;
  return read(notification_fd, &buf, 1);
}

long get_next_available_notification_cell() {
  notification_info* notification;
  // Check for empty cell till end of notification_infos 
  for(long i = job_notification_index + 1; i < job_notification_count; i++){
    notification = notification_infos[i];
    if(notification == NULL) return i;
  }
  
  // Check from start(0) till job_notification_index
  for(long i = 0; i < job_notification_index; i++){
    notification = notification_infos[i];
    if(notification == NULL) return i;
  }
...
CAMLprim value lwt_unix_get_next_available_job_notification_id() {
  if(job_notification_mode == NOTIFICATION_MODE_NOT_INITIALIZED){
      job_init_notifications();

      #if defined(HAVE_EVENTFD)
        job_notification_mode = NOTIFICATION_MODE_EVENTFD;
      #else
        job_notification_mode = NOTIFICATION_MODE_PIPE;
      #endif
  }

  long index_to_return = get_next_available_notification_cell();
  if(index_to_return == -1) {
    job_resize_notifications();
    index_to_return = last_job_notification_index + 1;
  }

  job_notification_index = index_to_return;
  if(job_notification_index > last_job_notification_index) last_job_notification_index = job_notification_index;
  return Val_long(job_notification_index);
}

CAMLprim value lwt_unix_init_job_notification(value val_notification_id) {
    
  intnat notification_id = Long_val(val_notification_id);
  
#if defined(HAVE_EVENTFD)
  int notification_fd = eventfd(0, 0);
  if (notification_fd != -1) {
    notification_info* notification = (notification_info *) lwt_unix_malloc(sizeof(notification_info));
    if(notification == NULL) caml_failwith("Failed to allocate nofification in lwt_unix_init_job_notification\n");
    notification->read_fd = notification_fd;
    notification->write_fd = notification_fd;
    notification->mode = NOTIFICATION_MODE_EVENTFD;
    notification->done = 0;
    set_close_on_exec(notification_fd);

    notification_infos[notification_id] = notification;
    return Val_int(notification_fd);
  }
#endif
  ...
  notification_info* notification = (notification_info *) lwt_unix_malloc(sizeof(notification_info));
  if(notification == NULL) caml_failwith("Failed to allocate nofification in lwt_unix_init_job_notification\n");
  notification->read_fd = notification_fds[0];
  notification->write_fd = notification_fds[1];
  notification->mode = NOTIFICATION_MODE_PIPE;
  notification->done = 0;
  set_close_on_exec(notification_fds[0]);
  set_close_on_exec(notification_fds[1]);
...
void send_job_notification(long id){
  #if !defined(LWT_ON_WINDOWS)
    sigset_t new_mask;
    sigset_t old_mask;
    int error;
  ...
  if(id >= job_notification_count)  caml_failwith("send_job_notification");
  notification_info* notification = notification_infos[id];
  if (notification == NULL) caml_failwith("trying to send empty notification");
  switch (notification->mode){
    case NOTIFICATION_MODE_EVENTFD:
      job_eventfd_notification_send(notification->write_fd);
      break;

    case NOTIFICATION_MODE_PIPE:
      job_pipe_notification_send(notification->write_fd);
      break;
    
    default:
        caml_failwith("unkown notification modem in send_job_notification");
  }
...
void lwt_send_job_notification(value id){
   send_job_notification(Long_val(id));
}

void close_fd(int fd){
  if (close(fd) == -1) uerror("close_fd", Nothing);
}

void lwt_unix_reset_and_close_notifications(){
  notification_info* notification;

  for (long i = 0; i <= last_job_notification_index; i++){
    notification = notification_infos[i];
    ...
    switch (notification->mode){
      case NOTIFICATION_MODE_EVENTFD:
        close_fd(notification->write_fd);
        break;

      case NOTIFICATION_MODE_PIPE:
        close_fd(notification->write_fd);
        close_fd(notification->read_fd);
        break;
      
      default:
          caml_failwith("lwt_unix_reset_and_close_notifications,");
    }
  }


  long total_notifications = last_job_notification_index;
  for (long i = 0; i <= total_notifications; i++){
    notification = notification_infos[i];
    ...
    if(notification->done) {
      ...
      /* If next job allocation index(i) is less than job_notification_index
      *   then update job_notification_index */
      if(i-1 < job_notification_index) job_notification_index = i - 1;
    } else{
      last_job_notification_index = i;
    }
  }
...
CAMLprim value lwt_unix_start_job(value val_job, value val_async_method, value val_notification_id) {
  ...
  switch (async_method) {
    ...
    case LWT_UNIX_ASYNC_METHOD_SWITCH:
      ...
      send_job_notification(notification_id);
