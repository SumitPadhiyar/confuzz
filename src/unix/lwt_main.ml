...
let exception_in_main = ref None
...
let fuzz_yield () = 
  if not (Lwt_sequence.is_empty yielded) then begin
    ...
    Lwt_sequence.iter_l (fun wakener -> 
        fuzz_list := (fun () -> Lwt.wakeup wakener ()) :: !fuzz_list)
      tmp;
    (* Printf.printf "yield callbacks - %d\n%!" (List.length !fuzz_list); *)
    Lwt_afl_scheduler.fuzz !fuzz_list
  end

let rec run t =
   match !exception_in_main with
   | Some e -> raise e
   | None ->
  ...
  | None ->
    ...
    run t
    with e -> exception_in_main := Some e; run t
